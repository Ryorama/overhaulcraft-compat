...
public abstract class NoiseBasedChunkGeneratorMixin extends ChunkGenerator {
    ...
    private Iterable<Holder<ConfiguredWorldCarver<?>>> filter(Iterable<Holder<ConfiguredWorldCarver<?>>> original) {
        if (IDimensionAccessor.of(getBiomeSource()).oc$isOverworld()) {
            return Iterables.filter(original, holder -> holder.getKey() != null && (!Confluence.MODID.equals(holder.getKey().location().getNamespace()) || !holder.getKey().location().getNamespace().contains("cobblemon")));
        }
        return original;
    }
    ...
    private SurfaceSystem apply(SurfaceSystem original) {
        if (IDimensionAccessor.of(getBiomeSource()).oc$isNotOverworld()) {
