...
@Mixin(HangingplayercorpseBlock.class)
public abstract class HangingplayercorpseBlockMixin extends BaseEntityBlock implements EntityBlock, IPlayerCorpse {
    @Unique
    public UUID playerUUID = new UUID(0L, 0L);

    protected HangingplayercorpseBlockMixin(Properties properties) {
        super(properties);
    }
    ...
    public BlockEntity newBlockEntity(BlockPos blockPos, BlockState blockState) {
        if (overhaulcraft_compat$getPlayerUUID() != null) {
            HangingplayercorpseTileEntity corpseEntity = ButcherModBlockEntities.HANGINGPLAYERCORPSE.get().create(blockPos, blockState);
            ((IPlayerCorpse)corpseEntity).overhaulcraft_compat$setPlayerUUID(this.overhaulcraft_compat$getPlayerUUID());
            return corpseEntity;
        }
        return ButcherModBlockEntities.HANGINGPLAYERCORPSE.get().create(blockPos, blockState);
    }
}